...
ErrVal drawFrame(uint32_t *pCurrentFrame, const uint32_t maxFramesInFlight,
	...
	ErrVal setCameraViewRetVal = copyToDeviceMemory(&(pModelViewProjectionBufferMemories[imageIndex]),
			sizeof(struct ModelViewProjectionMatrices),
			&modelViewProjectionMatrices,
			device);

	if(setCameraViewRetVal != VK_SUCCESS)
	{
		errLog(ERROR, "failed to set camera view, continuing with previous view");
	}
